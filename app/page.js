...
const GROUP_TAILWIND = [
  { bg: 'bg-violet-100', text: 'text-violet-700', border: 'border-violet-400' },
  { bg: 'bg-sky-100',    text: 'text-sky-700',    border: 'border-sky-400' },
...
function SetupScreen({ onDone }) {
  ...
  const submit = async () => {
    if (!form.username || !form.name || !form.password) { setError('All fields are required'); return }
    if (form.password !== form.confirm) { setError('Passwords do not match'); return }
    if (form.password.length < 8) { setError('Password must be at least 8 characters'); return }
    setSaving(true); setError('')
    try {
      await api('/api/setup', { method: 'POST', body: { username: form.username, name: form.name, password: form.password } })
      onDone()
        ...
        <div className="bg-white rounded-2xl p-8 shadow-2xl space-y-4">
          <div>
            ...
            <input className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="e.g. rebecca" value={form.username} onChange={e => set('username', e.target.value)} autoFocus />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Display Name</label>
            <input className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="e.g. Rebecca" value={form.name} onChange={e => set('name', e.target.value)} />
          </div>
          <div>
  ...
  return (
    <div className="w-52 min-h-screen flex flex-col bg-slate-900 text-white flex-shrink-0">
        ...
        {navItems.map(item => (
          <button key={item.id} onClick={() => setView(item.id)}
            className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm transition-colors text-left
              ${view === item.id ? 'bg-white/15 text-white' : 'text-slate-400 hover:bg-white/10 hover:text-white'}`}>
      ...
      <div className="p-4 border-t border-white/10 flex items-center justify-between">
        ...
        <button onClick={onLogout} className="text-slate-400 hover:text-white text-xs transition-colors">Sign out</button>
      </div>
    </div>
  )
}
...
function InboxView({ user, emails, setEmails, tickets, setTickets, groups, customers }) {
  ...
  const visible = emails.filter(e => {
    const gMatch = isAdmin ? (filterGroup === 'all' || e.group_id === Number(filterGroup)) : e.group_id === user.group_id
  ...
  const convertToTicket = async () => {
    ...
    try {
      ...
      const data = await api('/api/tickets', {
        method: 'POST',
        body: { subject: selectedEmail.subject, customer: cust?.name || selectedEmail.from_name, customerId: selectedEmail.customer_id, fromEmail: selectedEmail.from_email, groupId: selectedEmail.group_id, message: selectedEmail.body, assigneeId: user.id },
      })
    ...
    <div className="flex h-screen overflow-hidden">
      <div className="w-80 bg-white border-r border-gray-100 flex flex-col flex-shrink-0">
            ...
            <div className="flex gap-1.5">
              {['all', 'unread'].map(f => (
                <button key={f} onClick={() => setFilterRead(f)}
                  className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize transition-colors ${filterRead === f ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                  {f}
                </button>
              ))}
            </div>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
          ...
          {visible.map(e => {
            const g = gc(e.group_id)
            const gName = groups.find(x => x.id === e.group_id)?.name
            return (
              <button key={e.id} onClick={() => selectEmail(e.id)}
                ...
                {isAdmin && gName && <span className={`inline-block mt-1.5 text-xs px-2 py-0.5 rounded-full font-medium ${g.bg} ${g.text}`}>{gName}</span>}
              </button>
            )
          })}
        </div>
...
function TicketsView({ user, tickets, setTickets, groups }) {
  useTick()
  const [selected,       setSelected]       = useState(null)
  const [reply,          setReply]          = useState('')
  ...
  useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }) }, [selectedTicket?.messages?.length])
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      setTickets(prev => prev.map(t => t.id === selectedTicket.id ? { ...t, messages: [...t.messages, data.message], updated_at: data.message.created_at } : t))
      setReply('')
    } catch (e) { alert(e.message) }
    setSaving(false)
  }
  ...
  const addNote = async () => {
    ...
    try {
      ...
      setTickets(prev => prev.map(t => t.id === selectedTicket.id ? { ...t, notes: [...t.notes, data.note] } : t))
      setNote('')
    } catch (e) { alert(e.message) }
    setSaving(false)
  }
  ...
  const deleteTicket = async (id) => {
    if (!confirm(`Delete ticket ${id} permanently?`)) return
    try {
    ...
    <div className="flex h-screen overflow-hidden">
      <div className="w-80 bg-white border-r border-gray-100 flex flex-col flex-shrink-0">
            ...
            <div className="flex gap-1 flex-wrap">
              {['all','open','pending','resolved'].map(s => (
                <button key={s} onClick={() => setFilterStatus(s)}
                  className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize transition-colors ${filterStatus === s ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{s}</button>
              ))}
            </div>
            <div className="flex gap-1">
              {['all','high','medium','low'].map(p => (
                <button key={p} onClick={() => setFilterPriority(p)}
                  className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize transition-colors ${filterPriority === p ? 'bg-gray-700 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{p}</button>
              ))}
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
          ...
          {visible.map(t => {
            const g = gc(t.group_id)
            const gName = groups.find(x => x.id === t.group_id)?.name
            return (
              <button key={t.id} onClick={() => setSelected(t.id)}
                ...
                {isAdmin && gName && <span className={`mt-1.5 inline-block text-xs px-2 py-0.5 rounded-full font-medium ${g.bg} ${g.text}`}>{gName}</span>}
              </button>
            )
          })}
        </div>
      ...
      {selectedTicket ? (
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="w-60 border-l border-gray-100 flex flex-col bg-amber-50/40 flex-shrink-0">
              <div className="p-3 border-b border-gray-100">
                ...
                <p className="text-xs text-gray-500">Team only — not visible to customer</p>
              </div>
        ...
        <div className="flex-1 flex items-center justify-center bg-gray-50">
          <p className="text-gray-400 text-sm">Select a ticket</p>
        </div>
      )}
    </div>
  )
    ...
    <div className="flex h-screen overflow-hidden">
      <div className="w-72 bg-white border-r border-gray-100 flex flex-col flex-shrink-0">
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            {isAdmin && (
              <button onClick={() => setAdding(true)}
                className="text-xs bg-blue-600 text-white px-2.5 py-1 rounded-lg hover:bg-blue-700 transition-colors">+ Add</button>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {visible.length === 0 && <div className="p-8 text-center text-gray-400 text-sm">No customers</div>}
          {visible.map(c => {
            const g = gc(c.group_id)
            const gName = groups.find(x => x.id === c.group_id)?.name
            ...
            return (
              <button key={c.id} onClick={() => setSelected(c.id)}
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {gName && <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${g.bg} ${g.text}`}>{gName}</span>}
      ...
      {(adding || editing) ? (
        <CustomerForm initial={editing} groups={groups} onSave={saveCustomer} onCancel={() => { setAdding(false); setEditing(null) }} />
      ) : selectedCustomer ? (
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-0.5 flex-wrap text-xs text-gray-500">
                  {(() => { const g = gc(selectedCustomer.group_id); const gName = groups.find(x => x.id === selectedCustomer.group_id)?.name; return gName ? <span className={`px-2 py-0.5 rounded-full font-medium ${g.bg} ${g.text}`}>{gName}</span> : null })()}
              ...
              {isAdmin && (
                <div className="flex gap-2">
                  <button onClick={() => setEditing(selectedCustomer)} className="text-xs border border-gray-200 px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-colors">Edit</button>
                  <button onClick={() => deleteCustomer(selectedCustomer.id)} className="text-xs border border-red-200 text-red-600 bg-red-50 px-3 py-1.5 rounded-lg hover:bg-red-100 transition-colors">Delete</button>
                </div>
              )}
            </div>
        ...
        <div className="flex-1 flex items-center justify-center bg-gray-50">
          <p className="text-gray-400 text-sm">Select a customer</p>
        </div>
      )}
    </div>
  )
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
          ...
          <div>
            ...
            <input type="email" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={form.email} onChange={e => set('email', e.target.value)} placeholder="contact@company.com" />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={() => onSave({ ...form, groupId: form.groupId ? Number(form.groupId) : null })} disabled={!form.name.trim()}
...
function GroupsView({ tickets, emails, groups, customers }) {
  return (
    <div className="p-6 overflow-y-auto h-screen">
      ...
      {groups.length === 0 && <p className="text-gray-400 text-sm">No groups yet. Create one in Admin.</p>}
      <div className="grid grid-cols-2 gap-4 max-w-5xl">
        {groups.map(g => {
          ...
          return (
            <div key={g.id} className={`bg-white rounded-2xl border-t-4 ${g2.border} border border-gray-100 p-5`}>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-bold text-gray-900">{g.name}</h3>
                  <p className="text-sm text-gray-500 mt-0.5">{memberNames || 'No members assigned'}</p>
                  {g.email && <p className="text-xs text-gray-400 font-mono mt-0.5">{g.email}</p>}
                </div>
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg font-bold ${g2.bg} ${g2.text}`}>{g.id}</div>
              </div>
              <div className="grid grid-cols-4 gap-2 mb-4">
                <div className="text-center p-2 bg-blue-50 rounded-lg"><p className="text-lg font-bold text-blue-700">{open}</p><p className="text-xs text-blue-500">Open</p></div>
                <div className="text-center p-2 bg-orange-50 rounded-lg"><p className="text-lg font-bold text-orange-600">{pending}</p><p className="text-xs text-orange-400">Pending</p></div>
                <div className="text-center p-2 bg-green-50 rounded-lg"><p className="text-lg font-bold text-green-700">{resolved}</p><p className="text-xs text-green-500">Resolved</p></div>
                <div className="text-center p-2 bg-violet-50 rounded-lg"><p className="text-lg font-bold text-violet-700">{unread}</p><p className="text-xs text-violet-500">Unread</p></div>
              </div>
              <div>
                <p className="text-xs font-semibold text-gray-500 mb-2">CUSTOMERS ({gCustomers.length})</p>
                <div className="space-y-1">
                  {gCustomers.map(c => (
                    <div key={c.id} className={`flex items-center justify-between px-2.5 py-1.5 rounded-lg ${g2.bg}`}>
                      <span className={`text-xs font-medium ${g2.text}`}>{c.name}</span>
                      <span className={`text-xs ${g2.text} opacity-70`}>{c.plan}</span>
                    </div>
                  ))}
                  {gCustomers.length === 0 && <p className={`text-xs ${g2.text} opacity-60 px-2`}>No customers assigned</p>}
                </div>
              </div>
            </div>
          )
  ...
  const submit = async () => {
    ...
    try {
      if (form.mode === 'ticket') {
        ...
        const data = await api('/api/tickets', { method: 'POST', body: { subject: form.subject, customer: form.toName || form.to, customerId: cust?.id || null, fromEmail: form.to, groupId: form.groupId ? Number(form.groupId) : null, priority: form.priority, message: form.body } })
        setTickets(prev => [data.ticket, ...prev])
      } else {
        const cust = customers.find(c => c.email?.toLowerCase() === form.to.toLowerCase())
        const data = await api('/api/emails', { method: 'POST', body: { fromEmail: form.to, fromName: form.toName || form.to, subject: form.subject, emailBody: form.body, groupId: form.groupId ? Number(form.groupId) : null, customerId: cust?.id || null } })
...
function AdminView({ user, groups, setGroups, allUsers, setAllUsers, outlookStatus, onOutlookSync }) {
      ...
      <div className="flex gap-2 mb-6">
        {['users','groups','outlook','password'].map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === 'password' ? 'My Password' : t.charAt(0).toUpperCase() + t.slice(1)}
          </button>
        ))}
      </div>
...
function UsersTab({ groups, allUsers, setAllUsers, currentUser }) {
  ...
  const openEdit = (u) => { setEditing(u); setForm({ id: u.id, name: u.name, role: u.role, groupId: u.group_id || '', password: '', email: u.email || '' }); setAdding(false) }
  const openAdd  = ()  => { setEditing(null); setForm({ id:'', name:'', role:'agent', groupId:'', password:'', email:'' }); setAdding(true) }

  const save = async () => {
    try {
      const body = { name: form.name, role: form.role, groupId: form.groupId ? Number(form.groupId) : null, email: form.email }
      if (form.password) body.password = form.password
      if (adding) { body.id = form.id; body.password = form.password; const data = await api('/api/users', { method: 'POST', body }); setAllUsers(prev => [...prev, data.user]) }
      else { const data = await api(`/api/users/${editing.id}`, { method: 'PUT', body }); setAllUsers(prev => prev.map(u => u.id === editing.id ? data.user : u)) }
  ...
  const toggleActive = async (u) => {
    ...
    try {
      ...
      setAllUsers(prev => prev.map(x => x.id === u.id ? { ...x, active: u.active ? 0 : 1 } : x))
    } catch (e) { alert(e.message) }
  }
    ...
    <div className="max-w-4xl">
      ...
      {(adding || editing) && (
        <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {adding && <div><label className="block text-xs font-semibold text-gray-600 mb-1.5">USERNAME</label><input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. john" value={form.id} onChange={e => set('id', e.target.value)} /></div>}
            <div><label className="block text-xs font-semibold text-gray-600 mb-1.5">DISPLAY NAME</label><input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Full name" value={form.name} onChange={e => set('name', e.target.value)} /></div>
            <div><label className="block text-xs font-semibold text-gray-600 mb-1.5">EMAIL</label><input type="email" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="user@wellyx.com" value={form.email} onChange={e => set('email', e.target.value)} /></div>
            <div><label className="block text-xs font-semibold text-gray-600 mb-1.5">ROLE</label><select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form.role} onChange={e => set('role', e.target.value)}><option value="admin">Admin</option><option value="lead">Lead</option><option value="agent">Agent</option></select></div>
            <div><label className="block text-xs font-semibold text-gray-600 mb-1.5">GROUP</label><select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form.groupId} onChange={e => set('groupId', e.target.value)}><option value="">None</option>{groups.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}</select></div>
            <div><label className="block text-xs font-semibold text-gray-600 mb-1.5">{adding ? 'PASSWORD *' : 'NEW PASSWORD (blank = keep)'}</label><input type="password" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Min 8 characters" value={form.password} onChange={e => set('password', e.target.value)} /></div>
          </div>
          <div className="flex gap-3">
            <button onClick={save} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg text-sm font-medium transition-colors">{adding ? 'Add User' : 'Save Changes'}</button>
            <button onClick={() => { setAdding(false); setEditing(null) }} className="border border-gray-200 px-4 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors">Cancel</button>
          </div>
        </div>
      )}
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        {allUsers.length === 0 && <div className="p-8 text-center text-gray-400 text-sm">No users yet.</div>}
        {allUsers.length > 0 && (
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>{['User','Role','Group','Email','Status','Actions'].map(h => <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {allUsers.map(u => (
                <tr key={u.id} className={!u.active ? 'opacity-50' : ''}>
                  <td className="px-4 py-3"><div className="flex items-center gap-2"><Avatar name={u.name} size={7} bg="bg-blue-100" text="text-blue-700" /><div><p className="font-medium text-gray-800">{u.name}</p><p className="text-xs text-gray-400 font-mono">{u.id}</p></div></div></td>
                  <td className="px-4 py-3 capitalize text-gray-600">{u.role}</td>
                  <td className="px-4 py-3 text-gray-500">{groups.find(g => g.id === u.group_id)?.name || '—'}</td>
                  <td className="px-4 py-3 text-gray-500 text-xs">{u.email || '—'}</td>
                  <td className="px-4 py-3"><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${u.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{u.active ? 'Active' : 'Inactive'}</span></td>
                  <td className="px-4 py-3"><div className="flex gap-2"><button onClick={() => openEdit(u)} className="text-xs text-blue-600 hover:underline">Edit</button>{u.id !== currentUser.id && <button onClick={() => toggleActive(u)} className={`text-xs hover:underline ${u.active ? 'text-red-500' : 'text-green-600'}`}>{u.active ? 'Deactivate' : 'Reactivate'}</button>}</div></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function GroupsTab({ groups, setGroups }) {
  ...
  const save = async () => {
    try {
      if (adding) { const data = await api('/api/groups', { method: 'POST', body: form }); setGroups(prev => [...prev, data.group]) }
      else { const data = await api(`/api/groups/${editing.id}`, { method: 'PUT', body: form }); setGroups(prev => prev.map(g => g.id === editing.id ? data.group : g)) }
      setAdding(false); setEditing(null)
    } catch (e) { alert(e.message) }
  }
  ...
  const deleteGroup = async (g) => {
    ...
    try { await api(`/api/groups/${g.id}`, { method: 'DELETE' }); setGroups(prev => prev.filter(x => x.id !== g.id)) }
    catch (e) { alert(e.message) }
  }
    ...
    <div className="max-w-2xl">
      ...
      {(adding || editing) && (
        <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-xs font-semibold text-gray-600 mb-1.5">GROUP NAME</label><input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. Technical" value={form.name} onChange={e => set('name', e.target.value)} /></div>
            <div><label className="block text-xs font-semibold text-gray-600 mb-1.5">EMAIL ADDRESS</label><input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="group@wellyx.com" value={form.email} onChange={e => set('email', e.target.value)} /></div>
            <div><label className="block text-xs font-semibold text-gray-600 mb-1.5">COLOUR</label><div className="flex items-center gap-2"><input type="color" className="h-9 w-14 rounded border border-gray-200 cursor-pointer" value={form.color} onChange={e => set('color', e.target.value)} /><span className="text-sm text-gray-500 font-mono">{form.color}</span></div></div>
          </div>
          <div className="flex gap-3">
            <button onClick={save} disabled={!form.name.trim()} className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white px-5 py-2 rounded-lg text-sm font-medium transition-colors">{adding ? 'Add Group' : 'Save Changes'}</button>
            <button onClick={() => { setAdding(false); setEditing(null) }} className="border border-gray-200 px-4 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors">Cancel</button>
          </div>
        </div>
      )}
      {groups.length === 0 && !adding && <p className="text-gray-400 text-sm">No groups yet. Click Add Group to create one.</p>}
      <div className="space-y-3">
        {groups.map(g => {
          const g2 = gc(g.id)
          return (
            <div key={g.id} className={`bg-white rounded-xl border-l-4 ${g2.border} border border-gray-100 p-4 flex items-center gap-4`}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        ...
        {status?.configured && (
          <div className="space-y-3">
            ...
            {result && <p className={`text-sm ${result.ok ? 'text-green-600' : 'text-red-600'}`}>{result.msg}</p>}
          </div>
        )}
      </div>
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        ...
        <ol className="space-y-2 text-sm text-gray-600 list-decimal list-inside">
          <li>Go to <span className="font-mono text-xs bg-gray-100 px-1.5 py-0.5 rounded">portal.azure.com</span> and create a new App Registration</li>
          ...
          <li>Grant admin consent for both permissions</li>
          <li>Create a client secret under Certificates and secrets</li>
          <li>Copy the Client ID, Secret, and Tenant ID into your <span className="font-mono text-xs bg-gray-100 px-1.5 py-0.5 rounded">.env.local</span> file</li>
          ...
          <li>Restart the server and click Sync Now</li>
        </ol>
        <p className="text-xs text-gray-400 mt-4">Emails are auto-routed by matching the sender address to a known customer. Unknown senders appear unassigned.</p>
      </div>
    </div>
  )
}
...
export default function App() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (setupData.needsSetup) { setNeedsSetup(true) }
  ...
  const loadData = useCallback(async (u) => {
    ...
    try {
      const [gRes, tRes, eRes, cRes] = await Promise.all([
        api('/api/groups'), api('/api/tickets'), api('/api/emails'), api('/api/customers'),
      ])
      ...
      if (isAdmin) {
        const [uRes, oRes] = await Promise.all([ api('/api/users'), api('/api/outlook/sync') ])
  ...
  const isAdmin = user?.role === 'admin' || user?.role === 'lead'
  const unreadCount = emails.filter(e => (isAdmin || e.group_id === user?.group_id) && !e.read).length
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <main className="flex-1 overflow-hidden">
        {view === 'inbox'     && <InboxView   user={user} emails={emails} setEmails={setEmails} tickets={tickets} setTickets={setTickets} groups={groups} customers={customers} />}
        {view === 'tickets'   && <TicketsView user={user} tickets={tickets} setTickets={setTickets} groups={groups} />}
        {view === 'customers' && <CustomersView user={user} tickets={tickets} groups={groups} customers={customers} setCustomers={setCustomers} />}
        {view === 'groups'    && isAdmin && <GroupsView tickets={tickets} emails={emails} groups={groups} customers={customers} />}
        {view === 'compose'   && isAdmin && <ComposeView user={user} emails={emails} setEmails={setEmails} tickets={tickets} setTickets={setTickets} groups={groups} customers={customers} />}
        {view === 'admin'     && isAdmin && <AdminView user={user} groups={groups} setGroups={setGroups} allUsers={allUsers} setAllUsers={setAllUsers} outlookStatus={outlookStatus} onOutlookSync={() => loadData(user)} />}
      </main>
    </div>
  )
}
